    ...
    'columns' => [
        ...
        [
            ...
            'value' => function($model) use($postCount) {
                if($postCount != 0)
                {
                    return  $model->count . "<div class='statistics-grade-detail'>" . Yii::t('app', '{amount}% of all post', ['amount' => number_format(($model->count / $postCount) * 100, 2)]) . '<div>';
                }
            },
        ],
        [
...
<?= Html::beginPanel(Yii::t('app','Top post')) ?>
<?= GridView::widget([
